...
public class SqlOrderFinish {
	...
	public ArrayList<OrderFinish> getOrderResult() {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("select p.ProductName,o.quantity,p.Retail_Price, s.StallName from orders as o, member as m , stall as s, product as p where o.fk_userID=m.UserID AND s.storeID = o.storeID AND o.productID = p.productID AND ordertime >= NOW() - INTERVAL 30 SECOND;");
